          ...
          <div className="flex flex-wrap gap-2">
            {rows.map((room) => (
              <Button
                ...
                asChild
              >
                <Link href={`/monitoramento/salas?room=${room.id}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>
                  {room.name}
                </Link>
              </Button>
            ))}
          ...
          {selectedRoom ? (
            <>
              <div className="flex flex-wrap gap-3 text-sm">
                ...
                <span className="text-muted-foreground">Devices: {selectedRoom.deviceCount}</span>
                <span className="text-muted-foreground">
